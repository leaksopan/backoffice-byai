...
@section('content')
    <div class="space-y-6">
        <h2 class="text-xl font-semibold text-slate-900">Tambah Chart of Account</h2>
        ...
        <form action="{{ route('mdm.coa.store') }}" method="POST" class="space-y-4 rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            @csrf
                ...
                <div>
                    ...
                    <select name="normal_balance" required class="mt-1 block w-full rounded-md border-slate-300 shadow-sm">
                        @foreach($normalBalances as $nb)
                            <option value="{{ $nb }}" {{ old('normal_balance') == $nb ? 'selected' : '' }}>{{ ucfirst($nb) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" name="external_code" value="{{ old('external_code') }}" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm">
            </div>
            ...
            <div class="flex items-center gap-4">
                <label class="inline-flex items-center">
                    <input type="checkbox" name="is_header" value="1" {{ old('is_header') ? 'checked' : '' }} class="rounded border-slate-300">
