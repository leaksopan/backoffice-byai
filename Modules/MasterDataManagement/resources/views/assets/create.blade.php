...
@section('content')
    <div class="space-y-6">
        <h2 class="text-xl font-semibold text-slate-900">Tambah Aset</h2>
        ...
        <form action="{{ route('mdm.assets.store') }}" method="POST" class="space-y-4 rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            @csrf
                ...
                <div>
                    ...
                    <select name="condition" required class="mt-1 block w-full rounded-md border-slate-300 shadow-sm">
                        @foreach(['baik','rusak_ringan','rusak_berat'] as $c)
                            <option value="{{ $c }}" {{ old('condition') == $c ? 'selected' : '' }}>{{ str_replace('_', ' ', ucfirst($c)) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input type="number" name="acquisition_value" value="{{ old('acquisition_value') }}" required min="0" step="0.01" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700">Tanggal Perolehan</label>
                    <input type="date" name="acquisition_date" value="{{ old('acquisition_date') }}" required class="mt-1 block w-full rounded-md border-slate-300 shadow-sm">
                </div>
            </div>
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    <label class="block text-sm font-medium text-slate-700">Umur Ekonomis (tahun)</label>
                    <input type="number" name="useful_life_years" value="{{ old('useful_life_years') }}" min="1" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm">
                </div>
                <div>
                    ...
                    <select name="depreciation_method" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm">
                        ...
                        @foreach(['straight_line','declining_balance','units_of_production'] as $m)
                            <option value="{{ $m }}" {{ old('depreciation_method') == $m ? 'selected' : '' }}>{{ str_replace('_', ' ', ucfirst($m)) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div>
                ...
                <select name="current_location_id" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm">
                    ...
                    @foreach($locations as $l)
                        <option value="{{ $l->id }}" {{ old('current_location_id') == $l->id ? 'selected' : '' }}>{{ $l->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="block text-sm font-medium text-slate-700">Nilai Residu (Rp)</label>
                <input type="number" name="residual_value" value="{{ old('residual_value') }}" min="0" step="0.01" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm">
            </div>
            <div>
                <label class="block text-sm font-medium text-slate-700">Deskripsi</label>
                <textarea name="description" rows="2" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm">{{ old('description') }}</textarea>
            </div>
            <div>
