...
@section('content')
    <div class="space-y-6">
        <h2 class="text-xl font-semibold text-slate-900">Tambah SDM</h2>
        ...
        <form action="{{ route('mdm.human-resources.store') }}" method="POST" class="space-y-4 rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            @csrf
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    <label class="block text-sm font-medium text-slate-700">NIP</label>
                    <input type="text" name="nip" value="{{ old('nip') }}" required class="mt-1 block w-full rounded-md border-slate-300 shadow-sm">
                </div>
                <div>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="text" name="position" value="{{ old('position') }}" required class="mt-1 block w-full rounded-md border-slate-300 shadow-sm">
                </div>
            </div>
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <select name="employment_status" required class="mt-1 block w-full rounded-md border-slate-300 shadow-sm">
                        @foreach(['pns','pppk','kontrak','honorer'] as $s)
                            <option value="{{ $s }}" {{ old('employment_status') == $s ? 'selected' : '' }}>{{ ucfirst($s) }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700">Golongan</label>
                    <input type="text" name="grade" value="{{ old('grade') }}" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm">
                </div>
            </div>
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    <label class="block text-sm font-medium text-slate-700">Gaji Pokok</label>
                    <input type="number" name="basic_salary" value="{{ old('basic_salary') }}" min="0" step="0.01" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700">Jam Efektif/Minggu</label>
                    <input type="number" name="effective_hours_per_week" value="{{ old('effective_hours_per_week') }}" min="0" max="168" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm">
                </div>
            </div>
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    <label class="block text-sm font-medium text-slate-700">Tanggal Mulai Kerja</label>
                    <input type="date" name="hire_date" value="{{ old('hire_date') }}" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700">Tanggal Berakhir</label>
                    <input type="date" name="termination_date" value="{{ old('termination_date') }}" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm">
                </div>
            </div>
            <div>
