...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body
        class="min-h-screen app-bg text-slate-900 dark:text-slate-100"
        x-data="{ ...modulifyThemeState() }"
        x-init="initTheme()"
    >
        <div class="min-h-screen">
            <header class="border-b glass-surface glass-divider">
                <div class="flex w-full items-center justify-between gap-2 px-3 py-3 sm:px-4">
                    ...
                    <div class="flex items-center gap-2">
                        <button
                            class="glass-soft inline-flex h-10 w-10 items-center justify-center rounded-xl text-slate-700 hover:text-slate-900 dark:text-slate-200 dark:hover:text-slate-50"
                            type="button"
                            @click="toggleTheme"
